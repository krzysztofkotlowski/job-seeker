...
export function JobDetailPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => {
        toast.showError(e instanceof Error ? e.message : "Job not found");
        navigate("/jobs");
      })
      .finally(() => setLoading(false));
    api
      .detectedSkills(id)
      .then(setDetectedSkills)
      .catch((e) =>
        toast.showError(
          e instanceof Error ? e.message : "Failed to load detected skills",
        ),
      );
  }, [id, navigate, toast]);
  ...
  const handleStatusChange = async (status: JobStatus) => {
    setSaving(true);
    try {
      setJob(await api.updateJob(job.id, { status }));
    } catch (e) {
      toast.showError(
  ...
  const handleSaveNotes = async () => {
    setSaving(true);
    try {
      setJob(await api.updateJob(job.id, { notes }));
    } catch (e) {
      toast.showError(e instanceof Error ? e.message : "Failed to save notes");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleReposted = async () => {
    setSaving(true);
    try {
      setJob(await api.updateJob(job.id, { is_reposted: !job.is_reposted }));
    } catch (e) {
      toast.showError(e instanceof Error ? e.message : "Failed to update");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleSaved = async () => {
    setSaving(true);
    try {
      setJob(await api.updateJob(job.id, { saved: !job.saved }));
    } catch (e) {
      toast.showError(
        e instanceof Error ? e.message : "Failed to update saved status",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.showError(e instanceof Error ? e.message : "Failed to delete job");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <Paper sx={{ overflow: "hidden" }}>
        ...
        >
          <Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              {job.is_reposted && (
                <Chip label="Reposted" size="small" color="warning" />
              )}
            </Box>
      ...
      <Box sx={{ p: 3, display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        >
          ...
          {job.applied_date && (
            <InfoItem label="Applied" value={job.applied_date} />
          )}
          {job.date_expires && (
            <InfoItem label="Expires" value={job.date_expires} />
          )}
        </Box>
        ...
        {detectedSkills.length > 0 && (
          <Box>
            <Typography variant="subtitle2" fontWeight={600} sx={{ mb: 1 }}>
              ...
              >
                found in{" "}
                {detectedSkills.some((d) => d.source_field === "title")
                  ? "title & "
                  : ""}
                description
              </Typography>
...
function formatDescription(raw: string): string {
  ...
  const isHeader = (line: string) => {
    ...
    if (/^[A-ZŻŹĆĄŚĘŁÓŃ\s&/,\-–:()]+$/.test(trimmed) && trimmed.length > 3)
      return true;
