...
export function formatSalary(j: Job): {
  plnLine: string;
  ...
  const s = j.salary;
  if (!s || (!s.min && !s.max))
    return { plnLine: "\u2014", hourlyLine: null, originalLine: null };

  const cur = s.currency ?? "";
  const periodLabel =
    s.period === "hourly" ? "/h" : s.period === "daily" ? "/day" : "/mo";
  ...
  if (s.min_pln != null && s.max_pln != null) {
    ...
    const originalLine =
      cur !== "PLN" || s.period !== "monthly" ? originalStr : null;
...
export function formatSalaryLines(job: Job): {
  plnLine: string;
  hourlyLine: string | null;
  originalLine: string | null;
} {
  ...
  if (result.plnLine === "\u2014")
    return { ...result, plnLine: "Not specified" };
  return result;
}
...
export const STATUS_CONFIG: Record<
  ...
  {
    label: string;
    color: "primary" | "warning" | "success" | "error" | "info" | "default";
  }
> = {
